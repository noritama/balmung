...
function Dir(dirpath, options, config) {
...
Dir.prototype.collect = function(dirPath, callback) {
  ...
  var self = this;
  var list = [];
  var ignoredirs = this.config.ignoredirs || [];
  var ls = function(loc, callback) {
    self.ls(loc, function(err, result) {
      ...
      async.eachSeries(dirs, function(dir, done) {
        var _path = path.join(loc, dir);
        if (~ignoredirs.indexOf(_path)) {
          logger.info('Skip dir.', { dir: _path });
          return done();
        }
        ls(_path, done);
      }, function(err) {
 ...
 */
exports.add = function(name, path, config) {
  var dir = new Dir(path, null, config);
